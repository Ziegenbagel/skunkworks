...
def build():
    ...
    add_body(doc, "The Inventory & Containers page also provides manual controls for stock moves, resource or item jettison, container deployment to space, asteroids, discovered Others mothership wrecks, or planets, detached-object recovery, same-sector deuterium transfer, and Manny reassignment. Live jettison, deployment, recovery, fuel transfer, and reassignment orders require confirmation. A container with space reserved for active crafting output cannot be detached, dropped, transferred, or emptied until that craft completes or is cancelled; the game enforces this reservation and Skunkworks preserves its explanatory error.")
    add_note(doc, "Container operations", "A Manny can recover drifting containers and detected caches hidden on asteroids or Others mothership wrecks. Exhausted Others wrecks remain valid cache targets; other dormant constructs do not. Wreck caching is a manual operation and is not added to Miner automation. Whole containers can transfer directly to another owned probe in the same sector. Individual items still use jettison and salvage. Planet deployment consumes an Atmospheric Drop Kit, and transferring a busy Manny cancels its active task.")
...
def build_tabbed():
    ...
    add_body(doc, "Safety centralizes operational findings. Readiness distinguishes advisory notices from warning and critical conditions; a busy fleet can remain healthy even when no Manny is currently available. Mark All Persistent Alerts Read updates read status for the focused probe without deleting its alert history or changing sector damage warnings.")
    ...
    add_body(doc, "Deploy one attached container to space, an asteroid, a discovered Others mothership wreck, a planet, or another supported destination, then recover a visible drifting object or detected cache with a Manny. Routing rules control where newly received content is placed.")
